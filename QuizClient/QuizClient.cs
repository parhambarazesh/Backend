...
public class QuizClient
{
    ...
    // method to calculate the score of a quiz
    public Student CalculateScoreForStudent(int quizId,string studentName, IEnumerable<Question> questions, IEnumerable<Answer> answers)
    {
        var score = 0;
        foreach (var question in questions)
        {
            // check if Answer Id is the same as Question correct answer Id
            if (answers.Any(a => a.Id == question.CorrectAnswerId))
                score++;
        }
...
public class QuizClientException : HttpRequestException
{
    ...
    public QuizClientException() : base()
    {
    }

    public QuizClientException(string message) : base(message)
    {
    }

    public QuizClientException(string message, Exception inner) : base(message, inner)
    {
    }

    public QuizClientException(string message, Exception inner, HttpStatusCode? statusCode) : base(message, inner, statusCode)
    {
    }
